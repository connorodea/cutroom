...
export class CutroomClient {
  ...
  async pixelate(filePath: string, opts: { size?: "small" | "medium" | "large" } = {}): Promise<Job> {
    const fd = await this.fileForm(filePath, { size: opts.size ?? "medium" });
    return this.json(await fetch(`${this.baseUrl}/api/pixelate`, { method: "POST", headers: this.authHeaders(), body: fd }));
  }

  /** Vignette: darken the corners (subtle / medium / strong) for a cinematic look → job. */
  async vignette(filePath: string, opts: { strength?: "subtle" | "medium" | "strong" } = {}): Promise<Job> {
    const fd = await this.fileForm(filePath, { strength: opts.strength ?? "medium" });
    return this.json(await fetch(`${this.baseUrl}/api/vignette`, { method: "POST", headers: this.authHeaders(), body: fd }));
  }

  /** Progress bar: overlay an animated bottom bar that fills as the clip plays (color, thickness) → job. */
  async progress(filePath: string, opts: { color?: "cyan" | "magenta" | "lime" | "white" | "red"; thickness?: "thin" | "medium" | "thick" } = {}): Promise<Job> {
    const fd = await this.fileForm(filePath, { color: opts.color ?? "cyan", thickness: opts.thickness ?? "medium" });
    return this.json(await fetch(`${this.baseUrl}/api/progress`, { method: "POST", headers: this.authHeaders(), body: fd }));
  }
