...
export type IntentTool =
  ...
  | "thumbnail" | "stitch" | "watermark" | "pip" | "split" | "freeze" | "kenburns" | "chromakey" | "border" | "censor" | "music" | "grid" | "waveform" | "letterbox" | "subtitles" | "meme" | "progress" | "vignette" | "pixelate" | "rgbsplit" | null;
...
export function routeIntent(prompt: string): IntentTool {
  ...
  if (/\b(2\s*[x×]\s*2|grid|mosaic|tile\s+(four|4|the\s+clips)|four\s+(clips|videos)\s+(in|into))\b/.test(p)) {
    return "grid";
  }
  if (/\b(waveform|audiogram|audio\s?gram|visuali[sz]e\s+(the\s+)?audio|audio\s+visuali[sz]er)\b/.test(p)) {
    return "waveform";
  }
  if (/\b(letterbox|cinema\s?scope|cinemascope|black\s+bars|cinema\s+bars|movie\s+bars|bars\s+top\s+and\s+bottom)\b/.test(p)) {
    return "letterbox";
  }
  if (/\b(meme|impact\s+text|meme\s+text|meme\s+caption)\b/.test(p)) {
    return "meme";
  }
  if (/\b(progress\s?bar|playback\s+bar|seek\s?bar|progress\s+indicator|scrubber)\b/.test(p)) {
    return "progress";
  }
  ...
  if (/\b(glitch|rgb[\s-]?split|chromatic\s+aberration|channel\s+split|colou?r\s+split)\b/.test(p)) {
    return "rgbsplit";
  }
