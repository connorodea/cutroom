import { afterEach, beforeEach, describe, it, expect, vi } from "vitest";
import { act, fireEvent, render, screen } from "@testing-library/react";
import { useEditorStore } from "../editor/store";
import { GridModal } from "./GridModal";

function res(data: unknown, { ok = true, status = 200 }: { ok?: boolean; status?: number } = {}): Response {
  return { ok, status, json: async () => data } as unknown as Response;
}
const open = () => act(() => useEditorStore.setState({ gridOpen: true }));
let fetchMock: ReturnType<typeof vi.fn>;

beforeEach(() => {
  (URL as unknown as { createObjectURL: unknown }).createObjectURL = vi.fn(() => "blob:x");
  (URL as unknown as { revokeObjectURL: unknown }).revokeObjectURL = vi.fn();
  fetchMock = vi.fn();
  vi.stubGlobal("fetch", fetchMock);
});
afterEach(() => {
  vi.unstubAllGlobals();
  act(() => useEditorStore.setState(useEditorStore.getInitialState(), true));
});

const vid = (name: string) => new File(["v"], name, { type: "video/mp4" });
function setSlot(index: number, name: string) {
  const inputs = document.querySelectorAll('input[type="file"]');
  fireEvent.change(inputs[index] as HTMLInputElement, { target: { files: [vid(name)] } });
}
function fillAll() {
  setSlot(0, "a.mp4");
  setSlot(1, "b.mp4");
  setSlot(2, "c.mp4");
  setSlot(3, "d.mp4");
}

describe("GridModal", () => {
  it("renders nothing when closed", () => {
    render(<GridModal />);
    expect(screen.queryByText(/Top-left/)).not.toBeInTheDocument();
  });

  it("shows four slots when open", () => {
    open();
    render(<GridModal />);
    expect(screen.getByText("Top-left")).toBeInTheDocument();
    expect(screen.getByText("Top-right")).toBeInTheDocument();
    expect(screen.getByText("Bottom-left")).toBeInTheDocument();
    expect(screen.getByText("Bottom-right")).toBeInTheDocument();
  });

  it("requires all four clips before building", () => {
    open();
    render(<GridModal />);
    expect(screen.getByRole("button", { name: /Build grid/ })).toBeDisabled();
    setSlot(0, "a.mp4");
    setSlot(1, "b.mp4");
    setSlot(2, "c.mp4");
    expect(screen.getByRole("button", { name: /Build grid/ })).toBeDisabled();
    setSlot(3, "d.mp4");
    expect(screen.getByRole("button", { name: /Build grid/ })).toBeEnabled();
  });

  it("posts the four clips and shows the result", async () => {
    open();
    render(<GridModal />);
    fillAll();
    fetchMock
      .mockResolvedValueOnce(res({ id: "g1", status: "queued" }))
      .mockResolvedValueOnce(res({ id: "g1", status: "done", result: { outputId: "g1", cells: 4 } }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Build grid/ }));
    });
    const fd = fetchMock.mock.calls[0][1].body as FormData;
    expect(fd.getAll("files")).toHaveLength(4);
    expect(await screen.findByText(/Grid ready/)).toBeInTheDocument();
  });

  it("surfaces an error", async () => {
    open();
    render(<GridModal />);
    fillAll();
    fetchMock.mockResolvedValueOnce(res({ error: "bad" }, { ok: false, status: 400 }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Build grid/ }));
    });
    expect(await screen.findByText(/Grid failed/)).toBeInTheDocument();
  });

  it("surfaces a job-error status", async () => {
    open();
    render(<GridModal />);
    fillAll();
    fetchMock
      .mockResolvedValueOnce(res({ id: "g1", status: "queued" }))
      .mockResolvedValueOnce(res({ id: "g1", status: "error", error: "boom" }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Build grid/ }));
    });
    expect(await screen.findByText(/Grid failed/)).toBeInTheDocument();
  });

  it("clears the slots and closes via ✕", () => {
    open();
    render(<GridModal />);
    setSlot(0, "a.mp4");
    expect(screen.getByText("a.mp4")).toBeInTheDocument();
    fireEvent.click(screen.getByText("Clear"));
    expect(screen.queryByText("a.mp4")).not.toBeInTheDocument();
    fireEvent.click(screen.getByText("✕"));
    expect(useEditorStore.getState().gridOpen).toBe(false);
  });
});
