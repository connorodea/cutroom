...
import { outputUrl, pollJob, submitLetterboxJob, type EditJob } from "./workerClient";

const ACCENT = "#C0A062";
const PRESETS: { label: string; value: string }[] = [
  { label: "Cinema 2.39", value: "cinema" },
  { label: "Wide 2.0", value: "wide" },
  { label: "Classic 1.85", value: "classic" },
];
type Phase = "idle" | "submitting" | "running" | "done" | "error";

/** Overlay cinematic black bars on an uploaded clip at a target aspect ratio. */
export function LetterboxModal() {
  const open = useEditorStore((s) => s.letterboxOpen);
  const closeLetterbox = useEditorStore((s) => s.closeLetterbox);
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const submitted = await submitLetterboxJob(file, { preset });
      setJob(submitted);
      setPhase("running");
      ...
      if (final.status === "done") setPhase("done");
        ...
        setError(final.error || "letterbox failed");
        setPhase("error");
      }
    } catch (err) {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 17px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, background: "linear-gradient(150deg,#C0A062,#8C7038)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0C1012" }}>
            <Icon name="rectangle-horizontal" size={17} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Letterbox</div>
            <div style={{ fontSize: 11, color: "#79797F" }}>Add cinematic black bars</div>
          </div>
          {!busy && (
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          {!file && phase !== "done" && (
            <div
              ...
              style={{ border: `1.5px dashed ${dragOver ? ACCENT : "rgba(255,255,255,0.18)"}`, background: dragOver ? "rgba(192,160,98,0.06)" : "#161618", borderRadius: 14, padding: "44px 20px", textAlign: "center", cursor: "pointer" }}
            >
              <div style={{ width: 52, height: 52, borderRadius: 16, margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(192,160,98,0.12)", color: ACCENT }}>
                <Icon name="rectangle-horizontal" size={24} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Drop a video here, or click to choose</div>
              <div style={{ fontSize: 12, color: "#79797F", marginTop: 5 }}>We add black bars for a widescreen look</div>
          ...
          {file && phase !== "done" && (
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ fontSize: 12, color: "#79797F", marginRight: 2 }}>Ratio</span>
                {PRESETS.map((p) => (
                  <button
                    key={p.value}
                    ...
                    disabled={busy}
                    style={{ background: preset === p.value ? ACCENT : "#161618", color: preset === p.value ? "#0C1012" : "#C7C7CC", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: busy ? "default" : "pointer" }}
                  >
                    {p.label}
                  </button>
                ))}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                ...
                {busy ? (
                  <span style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: ACCENT }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(192,160,98,.3)", borderTopColor: ACCENT, animation: "spin .7s linear infinite", display: "block" }} />
                  ...
                  <>
                    ...
                    <button onClick={run} style={{ display: "flex", alignItems: "center", gap: 7, background: ACCENT, color: "#0C1012", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}>
                      <Icon name="rectangle-horizontal" size={14} />Letterbox
                    </button>
                  </>
                )}
              </div>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: ACCENT }}>
                  <Icon name="circle-check-big" size={15} />Bars added
                </span>
                <div style={{ flex: 1 }} />
