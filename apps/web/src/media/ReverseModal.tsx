...
import { outputUrl, pollJob, submitReverseJob, type EditJob } from "./workerClient";

const ACCENT = "#E0739B";
const MODES: { label: string; value: string; hint: string }[] = [
  { label: "Reverse", value: "reverse", hint: "play it backwards" },
  { label: "Boomerang", value: "boomerang", hint: "forward then back, on loop" },
];
type Phase = "idle" | "submitting" | "running" | "done" | "error";

/** Reverse an uploaded video, or turn it into a forward-then-reversed boomerang. */
export function ReverseModal() {
  const open = useEditorStore((s) => s.reverseOpen);
  const closeReverse = useEditorStore((s) => s.closeReverse);
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const submitted = await submitReverseJob(file, { mode });
      setJob(submitted);
      setPhase("running");
      ...
      if (final.status === "done") setPhase("done");
        ...
        setError(final.error || "reverse failed");
        setPhase("error");
      }
    } catch (err) {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 17px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, background: "linear-gradient(150deg,#E0739B,#B34E74)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0C1012" }}>
            <Icon name="rewind" size={17} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Reverse</div>
            <div style={{ fontSize: 11, color: "#79797F" }}>Play it backwards, or loop it as a boomerang</div>
          </div>
          {!busy && (
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          {!file && phase !== "done" && (
            <div
              ...
              style={{ border: `1.5px dashed ${dragOver ? ACCENT : "rgba(255,255,255,0.18)"}`, background: dragOver ? "rgba(224,115,155,0.06)" : "#161618", borderRadius: 14, padding: "44px 20px", textAlign: "center", cursor: "pointer" }}
            >
              <div style={{ width: 52, height: 52, borderRadius: 16, margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(224,115,155,0.12)", color: ACCENT }}>
                <Icon name="rewind" size={24} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Drop a video here, or click to choose</div>
              <div style={{ fontSize: 12, color: "#79797F", marginTop: 5 }}>Short clips work best</div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                {MODES.map((m) => (
                  <button
                    ...
                    disabled={busy}
                    style={{ background: mode === m.value ? ACCENT : "#161618", color: mode === m.value ? "#0C1012" : "#C7C7CC", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "5px 13px", fontSize: 12, fontWeight: 600, cursor: busy ? "default" : "pointer" }}
                  >
                    {m.label}
                  </button>
                ))}
                <span style={{ fontSize: 11.5, color: "#79797F" }}>{active.hint}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                ...
                {busy ? (
                  <span style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: ACCENT }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(224,115,155,.3)", borderTopColor: ACCENT, animation: "spin .7s linear infinite", display: "block" }} />
                  ...
                  <>
                    ...
                    <button onClick={run} style={{ display: "flex", alignItems: "center", gap: 7, background: ACCENT, color: "#0C1012", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}>
                      <Icon name="rewind" size={14} />Apply
                    </button>
                  </>
                )}
              </div>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: ACCENT }}>
                  <Icon name="circle-check-big" size={15} />{active.label} ready
                </span>
                <div style={{ flex: 1 }} />
