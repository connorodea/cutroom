...
import { outputUrl, pollJob, submitWatermarkJob, type EditJob } from "./workerClient";
...
/** Burn a persistent corner watermark (brand/attribution text) into an uploaded video. */
export function WatermarkModal() {
  const open = useEditorStore((s) => s.watermarkOpen);
  const closeWatermark = useEditorStore((s) => s.closeWatermark);
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const submitted = await submitWatermarkJob(file, { text: text.trim(), corner, opacity });
      setJob(submitted);
      setPhase("running");
      ...
      if (final.status === "done") setPhase("done");
        ...
        setError(final.error || "watermark failed");
        setPhase("error");
      }
    } catch (err) {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 17px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, background: "linear-gradient(150deg,#C9A24B,#9C7B2C)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0C1012" }}>
            <Icon name="type" size={17} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Watermark</div>
            <div style={{ fontSize: 11, color: "#79797F" }}>Brand the whole clip with corner text</div>
          </div>
          {!busy && (
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          {!file && phase !== "done" && (
            <div
              ...
              style={{ border: `1.5px dashed ${dragOver ? ACCENT : "rgba(255,255,255,0.18)"}`, background: dragOver ? "rgba(201,162,75,0.06)" : "#161618", borderRadius: 14, padding: "44px 20px", textAlign: "center", cursor: "pointer" }}
            >
              <div style={{ width: 52, height: 52, borderRadius: 16, margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(201,162,75,0.12)", color: ACCENT }}>
                <Icon name="type" size={24} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Drop a video here, or click to choose</div>
              <div style={{ fontSize: 12, color: "#79797F", marginTop: 5 }}>We stamp your text into a corner</div>
          ...
          {file && phase !== "done" && (
            <>
              ...
              <input
                ...
                style={{ width: "100%", marginTop: 12, background: "#161618", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, color: "#EDEDF0", padding: "9px 12px", fontSize: 13 }}
              />
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 12, color: "#79797F", margin: "0 2px 0 8px" }}>Opacity</span>
                {OPACITIES.map((o) => (
                  <button key={o.value} onClick={() => setOpacity(o.value)} aria-pressed={opacity === o.value} disabled={busy} style={chip(opacity === o.value)}>{o.label}</button>
                ))}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                ...
                {busy ? (
                  <span style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: ACCENT }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(201,162,75,.3)", borderTopColor: ACCENT, animation: "spin .7s linear infinite", display: "block" }} />
                    {phase === "submitting" ? "Uploading…" : "Stamping…"}
                  </span>
                ) : (
                  <>
                    ...
                    <button onClick={run} disabled={!ready} style={{ display: "flex", alignItems: "center", gap: 7, background: ready ? ACCENT : "#3A3A3D", color: ready ? "#0C1012" : "#79797F", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, fontWeight: 600, cursor: ready ? "pointer" : "default" }}>
                      <Icon name="type" size={14} />Add watermark
                    </button>
                  </>
                )}
              </div>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: ACCENT }}>
                  <Icon name="circle-check-big" size={15} />Watermarked
                </span>
                <div style={{ flex: 1 }} />
