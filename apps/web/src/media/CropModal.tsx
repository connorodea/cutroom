...
import { outputUrl, pollJob, submitCropJob, type EditJob } from "./workerClient";
...
/** Crop an uploaded video to a region of the frame. */
export function CropModal() {
  const open = useEditorStore((s) => s.cropOpen);
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const submitted = await submitCropJob(file, { preset });
      setJob(submitted);
      setPhase("running");
      ...
      if (final.status === "done") setPhase("done");
        ...
        setError(final.error || "crop failed");
        setPhase("error");
      }
    } catch (err) {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 17px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, background: "linear-gradient(150deg,#62B6A8,#3E8A7E)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0C1012" }}>
            <Icon name="crop" size={17} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Crop</div>
            <div style={{ fontSize: 11, color: "#79797F" }}>Punch into a region of the frame</div>
          </div>
          {!busy && (
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          {!file && phase !== "done" && (
            <div
              ...
              style={{ border: `1.5px dashed ${dragOver ? ACCENT : "rgba(255,255,255,0.18)"}`, background: dragOver ? "rgba(98,182,168,0.06)" : "#161618", borderRadius: 14, padding: "44px 20px", textAlign: "center", cursor: "pointer" }}
            >
              <div style={{ width: 52, height: 52, borderRadius: 16, margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(98,182,168,0.12)", color: ACCENT }}>
                <Icon name="crop" size={24} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Drop a video here, or click to choose</div>
              <div style={{ fontSize: 12, color: "#79797F", marginTop: 5 }}>Keep just the region you pick</div>
          ...
          {file && phase !== "done" && (
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ fontSize: 12, color: "#79797F", marginRight: 4 }}>Region</span>
                {PRESETS.map((p) => (
                  <button
                    key={p.value}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                ...
                {busy ? (
                  <span style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: ACCENT }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(98,182,168,.3)", borderTopColor: ACCENT, animation: "spin .7s linear infinite", display: "block" }} />
                    {phase === "submitting" ? "Uploading…" : "Cropping…"}
                  </span>
                ) : (
                  <>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: ACCENT }}>
                  <Icon name="circle-check-big" size={15} />{presetLabel} crop
                </span>
                <div style={{ flex: 1 }} />
