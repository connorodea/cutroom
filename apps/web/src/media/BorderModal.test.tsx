import { afterEach, beforeEach, describe, it, expect, vi } from "vitest";
import { act, fireEvent, render, screen } from "@testing-library/react";
import { useEditorStore } from "../editor/store";
import { BorderModal } from "./BorderModal";

function res(data: unknown, { ok = true, status = 200 }: { ok?: boolean; status?: number } = {}): Response {
  return { ok, status, json: async () => data } as unknown as Response;
}
const open = () => act(() => useEditorStore.setState({ borderOpen: true }));
let fetchMock: ReturnType<typeof vi.fn>;

beforeEach(() => {
  (URL as unknown as { createObjectURL: unknown }).createObjectURL = vi.fn(() => "blob:x");
  (URL as unknown as { revokeObjectURL: unknown }).revokeObjectURL = vi.fn();
  fetchMock = vi.fn();
  vi.stubGlobal("fetch", fetchMock);
});
afterEach(() => {
  vi.unstubAllGlobals();
  act(() => useEditorStore.setState(useEditorStore.getInitialState(), true));
});

function uploadVideo() {
  const input = document.querySelector('input[type="file"]') as HTMLInputElement;
  fireEvent.change(input, { target: { files: [new File(["v"], "clip.mp4", { type: "video/mp4" })] } });
}

describe("BorderModal", () => {
  it("renders nothing when closed", () => {
    render(<BorderModal />);
    expect(screen.queryByText(/Drop a video here/)).not.toBeInTheDocument();
  });

  it("shows the dropzone when open", () => {
    open();
    render(<BorderModal />);
    expect(screen.getByText(/Drop a video here/)).toBeInTheDocument();
  });

  it("highlights on drag and accepts a dropped file", () => {
    open();
    render(<BorderModal />);
    const zone = screen.getByText(/Drop a video here/).parentElement as HTMLElement;
    fireEvent.dragOver(zone);
    fireEvent.dragLeave(zone);
    fireEvent.drop(zone, { dataTransfer: { files: [new File(["v"], "dropped.mp4", { type: "video/mp4" })] } });
    expect(screen.getByText("dropped.mp4")).toBeInTheDocument();
  });

  it("offers the colors with White pre-selected", () => {
    open();
    render(<BorderModal />);
    uploadVideo();
    expect(screen.getByRole("button", { name: "White" })).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("button", { name: /Add border/ })).toBeInTheDocument();
  });

  it("sends the chosen color and thickness and shows the result", async () => {
    open();
    render(<BorderModal />);
    uploadVideo();
    fireEvent.click(screen.getByRole("button", { name: "Black" }));
    fireEvent.change(screen.getByLabelText("thickness"), { target: { value: "40" } });
    fetchMock
      .mockResolvedValueOnce(res({ id: "b1", status: "queued" }))
      .mockResolvedValueOnce(res({ id: "b1", status: "done", result: { outputId: "b1", borderColor: "black", borderThickness: 40, width: 400, height: 320 } }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Add border/ }));
    });
    const fd = fetchMock.mock.calls[0][1].body as FormData;
    expect(fd.get("color")).toBe("black");
    expect(fd.get("thickness")).toBe("40");
    expect(await screen.findByText(/Border added/)).toBeInTheDocument();
  });

  it("surfaces an error", async () => {
    open();
    render(<BorderModal />);
    uploadVideo();
    fetchMock.mockResolvedValueOnce(res({ error: "bad" }, { ok: false, status: 400 }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Add border/ }));
    });
    expect(await screen.findByText(/Border failed/)).toBeInTheDocument();
  });

  it("surfaces a job-error status", async () => {
    open();
    render(<BorderModal />);
    uploadVideo();
    fetchMock
      .mockResolvedValueOnce(res({ id: "b1", status: "queued" }))
      .mockResolvedValueOnce(res({ id: "b1", status: "error", error: "boom" }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Add border/ }));
    });
    expect(await screen.findByText(/Border failed/)).toBeInTheDocument();
  });

  it("changes the file via Change and closes via ✕", () => {
    open();
    render(<BorderModal />);
    uploadVideo();
    fireEvent.click(screen.getByText("Change"));
    expect(screen.getByText(/Drop a video here/)).toBeInTheDocument();
    fireEvent.click(screen.getByText("✕"));
    expect(useEditorStore.getState().borderOpen).toBe(false);
  });
});
