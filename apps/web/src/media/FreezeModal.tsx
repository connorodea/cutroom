...
import { outputUrl, pollJob, submitFreezeJob, type EditJob } from "./workerClient";

const ACCENT = "#6FC6D6";
const POSITIONS: { label: string; value: string }[] = [
  { label: "Hold start", value: "start" },
  { label: "Hold end", value: "end" },
];
type Phase = "idle" | "submitting" | "running" | "done" | "error";

/** Freeze-frame: hold the first or last frame of an uploaded video still for a few seconds. */
export function FreezeModal() {
  const open = useEditorStore((s) => s.freezeOpen);
  const closeFreeze = useEditorStore((s) => s.closeFreeze);
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const submitted = await submitFreezeJob(file, { position, seconds: parseFloat(seconds) || 2 });
      setJob(submitted);
      setPhase("running");
      ...
      if (final.status === "done") setPhase("done");
        ...
        setError(final.error || "freeze failed");
        setPhase("error");
      }
    } catch (err) {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 17px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, background: "linear-gradient(150deg,#6FC6D6,#3E92A6)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0C1012" }}>
            <Icon name="snowflake" size={17} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Freeze frame</div>
            <div style={{ fontSize: 11, color: "#79797F" }}>Hold the first or last frame still</div>
          </div>
          {!busy && (
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          {!file && phase !== "done" && (
            <div
              ...
              style={{ border: `1.5px dashed ${dragOver ? ACCENT : "rgba(255,255,255,0.18)"}`, background: dragOver ? "rgba(111,198,214,0.06)" : "#161618", borderRadius: 14, padding: "44px 20px", textAlign: "center", cursor: "pointer" }}
            >
              <div style={{ width: 52, height: 52, borderRadius: 16, margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(111,198,214,0.12)", color: ACCENT }}>
                <Icon name="snowflake" size={24} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Drop a video here, or click to choose</div>
              <div style={{ fontSize: 12, color: "#79797F", marginTop: 5 }}>We hold a frame so the moment lands</div>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12, flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12, color: "#79797F", marginRight: 2 }}>Hold</span>
                  {POSITIONS.map((p) => (
                    <button
                      key={p.value}
                      ...
                      disabled={busy}
                      style={{ background: position === p.value ? ACCENT : "#161618", color: position === p.value ? "#0C1012" : "#C7C7CC", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: busy ? "default" : "pointer" }}
                    >
                      {p.label}
                    </button>
                  ))}
                ...
                <label style={{ fontSize: 12, color: "#79797F", display: "flex", alignItems: "center", gap: 6 }}>
                  Seconds
                  <input aria-label="seconds" type="number" min={0.5} max={10} step={0.5} value={seconds} disabled={busy} onChange={(e) => setSeconds(e.target.value)} style={{ width: 70, background: "#161618", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "#EDEDF0", padding: "6px 9px", fontSize: 12.5 }} />
                </label>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                ...
                {busy ? (
                  <span style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: ACCENT }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(111,198,214,.3)", borderTopColor: ACCENT, animation: "spin .7s linear infinite", display: "block" }} />
                    {phase === "submitting" ? "Uploading…" : "Freezing…"}
                  </span>
                ) : (
                  <>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: ACCENT }}>
                  <Icon name="circle-check-big" size={15} />{posLabel} held
                </span>
                <div style={{ flex: 1 }} />
