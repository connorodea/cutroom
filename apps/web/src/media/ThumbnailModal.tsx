...
import { outputUrl, pollJob, submitThumbnailJob, type EditJob } from "./workerClient";

const ACCENT = "#6FA8DC";
type Phase = "idle" | "submitting" | "running" | "done" | "error";

/** Grab a poster frame from an uploaded video — scrub the preview, then capture. */
export function ThumbnailModal() {
  const open = useEditorStore((s) => s.thumbnailOpen);
  const closeThumbnail = useEditorStore((s) => s.closeThumbnail);
  ...
  const run = async () => {
    if (!file) return;
    const time = Math.round((videoRef.current?.currentTime ?? 0) * 100) / 100;
    setPhase("submitting");
    setError(null);
    try {
      const submitted = await submitThumbnailJob(file, { time });
      setJob(submitted);
      setPhase("running");
      ...
      if (final.status === "done") setPhase("done");
        ...
        setError(final.error || "thumbnail failed");
        setPhase("error");
      }
    } catch (err) {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 17px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, background: "linear-gradient(150deg,#6FA8DC,#4A7FB0)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0C1012" }}>
            <Icon name="camera" size={17} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Thumbnail</div>
            <div style={{ fontSize: 11, color: "#79797F" }}>Grab a poster frame as a PNG</div>
          </div>
          {!busy && (
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          {!file && phase !== "done" && (
            <div
              ...
              style={{ border: `1.5px dashed ${dragOver ? ACCENT : "rgba(255,255,255,0.18)"}`, background: dragOver ? "rgba(111,168,220,0.06)" : "#161618", borderRadius: 14, padding: "44px 20px", textAlign: "center", cursor: "pointer" }}
            >
              <div style={{ width: 52, height: 52, borderRadius: 16, margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(111,168,220,0.12)", color: ACCENT }}>
                <Icon name="camera" size={24} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Drop a video here, or click to choose</div>
              <div style={{ fontSize: 12, color: "#79797F", marginTop: 5 }}>Scrub to the frame you want, then capture</div>
          ...
          {file && phase !== "done" && (
            <>
              <video ref={videoRef} src={previewUrl ?? undefined} controls style={{ width: "100%", borderRadius: 12, background: "#000", maxHeight: 300 }} />
              <div style={{ fontSize: 11.5, color: "#79797F", marginTop: 8 }}>Scrub the video to the moment you want, then capture that frame.</div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                ...
                {busy ? (
                  <span style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: ACCENT }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(111,168,220,.3)", borderTopColor: ACCENT, animation: "spin .7s linear infinite", display: "block" }} />
                    {phase === "submitting" ? "Uploading…" : "Grabbing frame…"}
                  </span>
                ) : (
                  <>
                    ...
                    <button onClick={run} style={{ display: "flex", alignItems: "center", gap: 7, background: ACCENT, color: "#0C1012", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}>
                      <Icon name="camera" size={14} />Capture frame
                    </button>
                  </>
                )}
              </div>
          ...
          {phase === "done" && result && (
            <>
              <img src={outputUrl(result.outputId)} alt="poster frame" style={{ width: "100%", borderRadius: 12, background: "#000", maxHeight: 360, objectFit: "contain" }} />
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: ACCENT }}>
                  <Icon name="circle-check-big" size={15} />Poster grabbed at {result.time}s
                </span>
                <div style={{ flex: 1 }} />
                ...
                <a href={outputUrl(result.outputId)} download={`cutroom-${result.outputId}.png`} style={{ display: "flex", alignItems: "center", gap: 7, background: ACCENT, color: "#0C1012", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, fontWeight: 600, textDecoration: "none" }}>
