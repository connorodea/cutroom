...
import { outputUrl, pollJob, submitFadeJob, type EditJob } from "./workerClient";
...
/** Add an intro/outro fade-from/to-black to an uploaded video. */
export function FadeModal() {
  const open = useEditorStore((s) => s.fadeOpen);
  const closeFade = useEditorStore((s) => s.closeFade);
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const submitted = await submitFadeJob(file, { kind, duration: parseFloat(duration) || 0.5 });
      setJob(submitted);
      setPhase("running");
      ...
      if (final.status === "done") setPhase("done");
        ...
        setError(final.error || "fade failed");
        setPhase("error");
      }
    } catch (err) {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 17px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, background: "linear-gradient(150deg,#8AA0B6,#5E748B)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0C1012" }}>
            <Icon name="contrast" size={17} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Fade</div>
            <div style={{ fontSize: 11, color: "#79797F" }}>Ease in from black and/or out to black</div>
          </div>
          {!busy && (
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          {!file && phase !== "done" && (
            <div
              ...
              style={{ border: `1.5px dashed ${dragOver ? ACCENT : "rgba(255,255,255,0.18)"}`, background: dragOver ? "rgba(138,160,182,0.06)" : "#161618", borderRadius: 14, padding: "44px 20px", textAlign: "center", cursor: "pointer" }}
            >
              <div style={{ width: 52, height: 52, borderRadius: 16, margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(138,160,182,0.12)", color: ACCENT }}>
                <Icon name="contrast" size={24} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Drop a video here, or click to choose</div>
              <div style={{ fontSize: 12, color: "#79797F", marginTop: 5 }}>We ease the clip in/out of black</div>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12, flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12, color: "#79797F", marginRight: 2 }}>Fade</span>
                  {KINDS.map((k) => (
                    <button
                      key={k.value}
                      ...
                      disabled={busy}
                      style={{ background: kind === k.value ? ACCENT : "#161618", color: kind === k.value ? "#0C1012" : "#C7C7CC", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: busy ? "default" : "pointer" }}
                    >
                      {k.label}
                    </button>
                  ))}
                ...
                <label style={{ fontSize: 12, color: "#79797F", display: "flex", alignItems: "center", gap: 6 }}>
                  Seconds
                  <input aria-label="duration" type="number" min={0.1} step={0.1} value={duration} disabled={busy} onChange={(e) => setDuration(e.target.value)} style={{ width: 70, background: "#161618", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "#EDEDF0", padding: "6px 9px", fontSize: 12.5 }} />
                </label>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                ...
                {busy ? (
                  <span style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: ACCENT }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(138,160,182,.3)", borderTopColor: ACCENT, animation: "spin .7s linear infinite", display: "block" }} />
                    {phase === "submitting" ? "Uploading…" : "Fading…"}
                  </span>
                ) : (
                  <>
                    ...
                    <button onClick={run} style={{ display: "flex", alignItems: "center", gap: 7, background: ACCENT, color: "#0C1012", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}>
                      <Icon name="contrast" size={14} />Add fade
                    </button>
                  </>
                )}
              </div>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: ACCENT }}>
                  <Icon name="circle-check-big" size={15} />Fade {kindLabel.toLowerCase()} applied
                </span>
                <div style={{ flex: 1 }} />
