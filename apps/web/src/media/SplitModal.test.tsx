import { afterEach, beforeEach, describe, it, expect, vi } from "vitest";
import { act, fireEvent, render, screen } from "@testing-library/react";
import { useEditorStore } from "../editor/store";
import { SplitModal } from "./SplitModal";

function res(data: unknown, { ok = true, status = 200 }: { ok?: boolean; status?: number } = {}): Response {
  return { ok, status, json: async () => data } as unknown as Response;
}
const open = () => act(() => useEditorStore.setState({ splitOpen: true }));
let fetchMock: ReturnType<typeof vi.fn>;

beforeEach(() => {
  (URL as unknown as { createObjectURL: unknown }).createObjectURL = vi.fn(() => "blob:x");
  (URL as unknown as { revokeObjectURL: unknown }).revokeObjectURL = vi.fn();
  fetchMock = vi.fn();
  vi.stubGlobal("fetch", fetchMock);
});
afterEach(() => {
  vi.unstubAllGlobals();
  act(() => useEditorStore.setState(useEditorStore.getInitialState(), true));
});

const vid = (name: string) => new File(["v"], name, { type: "video/mp4" });
function setSlot(index: number, name: string) {
  const inputs = document.querySelectorAll('input[type="file"]');
  fireEvent.change(inputs[index] as HTMLInputElement, { target: { files: [vid(name)] } });
}

describe("SplitModal", () => {
  it("renders nothing when closed", () => {
    render(<SplitModal />);
    expect(screen.queryByText(/Left clip/)).not.toBeInTheDocument();
  });

  it("shows the two side-by-side slots when open", () => {
    open();
    render(<SplitModal />);
    expect(screen.getByText("Left clip")).toBeInTheDocument();
    expect(screen.getByText("Right clip")).toBeInTheDocument();
  });

  it("relabels the slots when stacked", () => {
    open();
    render(<SplitModal />);
    fireEvent.click(screen.getByRole("button", { name: "Stacked" }));
    expect(screen.getByText("Top clip")).toBeInTheDocument();
    expect(screen.getByText("Bottom clip")).toBeInTheDocument();
  });

  it("requires two clips before combining", () => {
    open();
    render(<SplitModal />);
    expect(screen.getByRole("button", { name: /Combine/ })).toBeDisabled();
    setSlot(0, "left.mp4");
    expect(screen.getByRole("button", { name: /Combine/ })).toBeDisabled();
    setSlot(1, "right.mp4");
    expect(screen.getByRole("button", { name: /Combine/ })).toBeEnabled();
  });

  it("posts the chosen layout and shows the result", async () => {
    open();
    render(<SplitModal />);
    setSlot(0, "left.mp4");
    setSlot(1, "right.mp4");
    fireEvent.click(screen.getByRole("button", { name: "Stacked" }));
    fetchMock
      .mockResolvedValueOnce(res({ id: "s1", status: "queued" }))
      .mockResolvedValueOnce(res({ id: "s1", status: "done", result: { outputId: "s1", layout: "vertical" } }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Combine/ }));
    });
    const fd = fetchMock.mock.calls[0][1].body as FormData;
    expect(fd.get("file")).toBeInstanceOf(File);
    expect(fd.get("right")).toBeInstanceOf(File);
    expect(fd.get("layout")).toBe("vertical");
    expect(await screen.findByText(/Split-screen ready/)).toBeInTheDocument();
  });

  it("surfaces an error", async () => {
    open();
    render(<SplitModal />);
    setSlot(0, "left.mp4");
    setSlot(1, "right.mp4");
    fetchMock.mockResolvedValueOnce(res({ error: "bad" }, { ok: false, status: 400 }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Combine/ }));
    });
    expect(await screen.findByText(/Split-screen failed/)).toBeInTheDocument();
  });

  it("surfaces a job-error status", async () => {
    open();
    render(<SplitModal />);
    setSlot(0, "left.mp4");
    setSlot(1, "right.mp4");
    fetchMock
      .mockResolvedValueOnce(res({ id: "s1", status: "queued" }))
      .mockResolvedValueOnce(res({ id: "s1", status: "error", error: "boom" }));
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /Combine/ }));
    });
    expect(await screen.findByText(/Split-screen failed/)).toBeInTheDocument();
  });

  it("clears the slots and closes via ✕", () => {
    open();
    render(<SplitModal />);
    setSlot(0, "left.mp4");
    expect(screen.getByText("left.mp4")).toBeInTheDocument();
    fireEvent.click(screen.getByText("Clear"));
    expect(screen.queryByText("left.mp4")).not.toBeInTheDocument();
    fireEvent.click(screen.getByText("✕"));
    expect(useEditorStore.getState().splitOpen).toBe(false);
  });
});
