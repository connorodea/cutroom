...
import { outputUrl, pollJob, submitSubtitlesJob, type EditJob } from "./workerClient";

const ACCENT = "#E0C24A";
type Phase = "idle" | "submitting" | "running" | "done" | "error";

/** Burn a user-supplied .srt subtitle file onto a clip. */
export function SubtitlesModal() {
  const open = useEditorStore((s) => s.subtitlesOpen);
  const closeSubtitles = useEditorStore((s) => s.closeSubtitles);
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const submitted = await submitSubtitlesJob(video, srt);
      setJob(submitted);
      setPhase("running");
      ...
      if (final.status === "done") setPhase("done");
        ...
        setError(final.error || "subtitles failed");
        setPhase("error");
      }
    } catch (err) {
  ...
  const slot = (label: string, accept: string, file: File | null, onPick: (f: File | null) => void, ref: React.RefObject<HTMLInputElement | null>) => (
    <div
      ...
      style={{ flex: 1, minWidth: 0, border: `1.5px dashed ${file ? "rgba(224,194,74,0.5)" : "rgba(255,255,255,0.18)"}`, background: "#161618", borderRadius: 12, padding: "20px 14px", textAlign: "center", cursor: "pointer" }}
    >
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 17px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, background: "linear-gradient(150deg,#E0C24A,#A88E22)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0C1012" }}>
            <Icon name="captions" size={17} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600 }}>Burn subtitles</div>
            <div style={{ fontSize: 11, color: "#79797F" }}>Bake your .srt file into the video</div>
          </div>
          {!busy && (
          ...
          {phase !== "done" && (
            <>
              <div style={{ display: "flex", gap: 12 }}>
                ...
                {slot("Subtitles (.srt)", ".srt,text/plain", srt, setSrt, srtRef)}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
                {!ready && <span style={{ fontSize: 11.5, color: "#E0A33E" }}>Add a clip and a .srt file</span>}
                <div style={{ flex: 1 }} />
                {busy ? (
                  <span style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: ACCENT }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid rgba(224,194,74,.3)", borderTopColor: ACCENT, animation: "spin .7s linear infinite", display: "block" }} />
                  ...
                  <>
                    ...
                    <button onClick={run} disabled={!ready} style={{ display: "flex", alignItems: "center", gap: 7, background: ready ? ACCENT : "#3A3A3D", color: ready ? "#0C1012" : "#79797F", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 12.5, fontWeight: 600, cursor: ready ? "pointer" : "default" }}>
                      <Icon name="captions" size={14} />Burn in
                    </button>
                  </>
                )}
              </div>
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: ACCENT }}>
                  <Icon name="circle-check-big" size={15} />{result.cues ?? 0} subtitles burned in
                </span>
                <div style={{ flex: 1 }} />
