...
/**
 * Create + run a Music job: mix a music track under a clip at a chosen volume. The output is
 * `${jobId}.mp4`. Runs async; poll the job for status.
 */
export function createMusicJob(videoPath: string, audioPath: string, volume: unknown, workDir: string): Job {
  const id = randomUUID();
  const job: Job = { id, type: "music", status: "queued", createdAt: Date.now() };
  jobs.set(id, job);
  void (async () => {
    try {
      job.status = "running";
...
/**
 * Create + run a Waveform job: render an audio file into a waveform video. The output is
 * `${jobId}.mp4`. Runs async; poll the job for status.
 */
export function createWaveformJob(audioPath: string, mode: unknown, color: unknown, aspect: unknown, workDir: string): Job {
  const id = randomUUID();
  const job: Job = { id, type: "waveform", status: "queued", createdAt: Date.now() };
  jobs.set(id, job);
  void (async () => {
    try {
      job.status = "running";
      job.step = "render waveform";
      const r = await runWaveformPipeline(audioPath, mode, color, aspect, workDir, id);
      job.status = "done";
      job.result = { outputId: id, waveMode: r.mode, waveColor: r.color, width: r.width, height: r.height };
    } catch (err) {
      job.status = "error";
      job.error = (err as Error).message;
      // eslint-disable-next-line no-console
...
/**
 * Create + run a Letterbox job: overlay cinematic black bars at a target aspect ratio. The output is
 * `${jobId}.mp4`. Runs async; poll the job for status.
 */
export function createLetterboxJob(inputPath: string, preset: unknown, workDir: string): Job {
  const id = randomUUID();
  const job: Job = { id, type: "letterbox", status: "queued", createdAt: Date.now() };
  jobs.set(id, job);
  void (async () => {
    try {
      job.status = "running";
      job.step = "draw cinematic bars";
...
/**
 * Create + run a Pixelate job: reduce a clip to chunky mosaic blocks. The output is `${jobId}.mp4`.
 * Runs async; poll the job for status.
 */
export function createPixelateJob(inputPath: string, size: unknown, workDir: string): Job {
  const id = randomUUID();
  const job: Job = { id, type: "pixelate", status: "queued", createdAt: Date.now() };
  jobs.set(id, job);
  void (async () => {
    try {
      job.status = "running";
